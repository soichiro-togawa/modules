...
def test(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader(path, imgsz, batch_size, model.stride.max(), opt, pad=0.5, rect=True,li=li)[0]
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(output):
            ...
            ########################引数、元座標うけとり
            ex_coords = scale_coords(img[si].shape[1:], predn[:, :4], shapes[si][0], shapes[si][1])  # native-space pred
            ###############################追加
            # continue
        time_veri.end_point(time_veri.t_point4)
        time_veri2.end_point(time_veri2.t_point0)
        logger.info("gpu_batch:{}".format(gpu_used.gpuinfo()))
...
#####################################追加##################################
    ...
    logger.info("①～④の処理時間:{}sum：{}".format(time_veri.time_sum_list(), time_veri.all))
...
if __name__ == '__main__':
    ...
    print(opt.data, opt.save_json)
    opt.data = check_file(opt.data)  # check file
    print(opt)
    logger.info("arg:{}".format(opt))
